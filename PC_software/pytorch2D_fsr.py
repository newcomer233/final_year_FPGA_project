...
class GestureDatasetSegments(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 读 CSV，只取前 channels 列
        df = pd.read_csv(path)
        x = df.iloc[:, :self.channels].values.astype(np.float32)  # (L, C)，只取前6列
...
def train_model(
    ...
    plt.xlabel('Epoch'); plt.legend(); plt.grid(); plt.title('Training Loss & Validation Acc (segment) fsr')
    plt.savefig('training_curve_100_fsr.png')

    # 混淆矩阵（按段统计）
    cm = confusion_matrix(trues, preds)
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=LABEL_NAMES)
    disp.plot(cmap='Blues', xticks_rotation=45)
    plt.title('Validation Confusion Matrix (per-segment) fsr')
    plt.savefig('confmat_100_fsr.png')
    print("✅ 图表已保存：training_curve_100_fsr.png, confmat_100_fsr.png")

    # 保存模型
    weight_path = 'resnet2d_100depth_weights_fsr.pth'
    ...
    torch.onnx.export(
        model, dummy, "resnet2d_100depth_fsr.onnx",
        input_names=['input'], output_names=['output'],
        dynamic_axes={'input': {0: 'batch'}, 'output': {0: 'batch'}},
        opset_version=11
    )
    print("✅ 已导出 ONNX：resnet2d_100depth_fsr.onnx")
