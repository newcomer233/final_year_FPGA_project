...
class GestureDatasetSegments(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 读 CSV，只取后 channels 列
        df = pd.read_csv(path)
        x = df.iloc[:, -self.channels:].values.astype(np.float32)  # (L, C)，只取最后6列

        # 截断/补齐到 total_len × channels
        if x.shape[0] < self.total_len:
            pad = np.zeros((self.total_len - x.shape[0], self.channels), dtype=np.float32)
            x = np.concatenate([x, pad], axis=0)
        x = x[:self.total_len, :]  # (total_len, C)

        # 取该段的 seg_len 点片段
        st = sidx * self.seg_len
        ed = st + self.seg_len
        seg = x[st:ed, :]                 # (seg_len, C)

        # 转成 [1, C, seg_len]
        seg = seg.T                       # (C, seg_len)
        seg = torch.from_numpy(seg).unsqueeze(0)  # [1, C, seg_len]
...
def train_model(
    ...
    plt.xlabel('Epoch'); plt.legend(); plt.grid(); plt.title('Training Loss & Validation Acc (segment) imu')
    plt.savefig('training_curve_100_imu.png')

    # 混淆矩阵（按段统计）
    cm = confusion_matrix(trues, preds)
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=LABEL_NAMES)
    disp.plot(cmap='Blues', xticks_rotation=45)
    plt.title('Validation Confusion Matrix (per-segment) imu')
    plt.savefig('confmat_100_imu.png')
    print("✅ 图表已保存：training_curve_100_imu.png, confmat_100_imu.png")

    # 保存模型
    weight_path = 'resnet2d_100depth_weights_imu.pth'
    ...
    torch.onnx.export(
        model, dummy, "resnet2d_100depth_imu.onnx",
        input_names=['input'], output_names=['output'],
        dynamic_axes={'input': {0: 'batch'}, 'output': {0: 'batch'}},
        opset_version=11
    )
    print("✅ 已导出 ONNX：resnet2d_100depth_imu.onnx")
